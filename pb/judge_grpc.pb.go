...
type ExecutorClient interface {
	// Exec 定义了一元 RPC，用于在资源限制下运行程序
	Exec(ctx context.Context, in *Request, opts ...grpc.CallOption) (*Response, error)
	// ExecStream 定义了流式 RPC，用于运行具有实时输入和输出的程序。
	// 第一个请求必须为 execRequest，后续请求必须为 execInput，
	// 最后一个响应必须为 execResponse，其余为 execOutput。
	// 当设置 TTY 属性时，将为程序创建单个伪终端，
	// 因此 stdout 和 stderr 应具有相同的名称
	ExecStream(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[StreamRequest, StreamResponse], error)
	// FileList 列出文件存储中所有可用的文件
